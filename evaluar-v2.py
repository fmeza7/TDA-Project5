...
def load_dense_labels(filename):
    """
    Carga un archivo de etiquetas densas.
    Se espera el formato: video_name \t frame_index \t label_name_or_id
    Retorna un diccionario: {video_name: np.array(labels)}
    """
    if not os.path.isfile(filename):
        raise Exception(f"No existe el archivo {filename}")
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            video = parts[0].strip()
            # El frame_idx se ignora para el guardado asumiendo que vienen en orden
            label = parts[2].strip()
            
            if video not in data_by_video:
                data_by_video[video] = []
                
            data_by_video[video].append(label)
            
    # Convertir listas a numpy arrays
    for video in data_by_video:
        data_by_video[video] = np.array(data_by_video[video])
        
    return data_by_video

def evaluar_segmentacion(pred_file, gt_file):
    print("Cargando predicciones...")
    preds_dict = load_dense_labels(pred_file)
    
    print("Cargando Ground Truth...")
    gt_dict = load_dense_labels(gt_file)
    
    all_preds = []
    all_gts = []
    
    for video in gt_dict.keys():
        if video not in preds_dict:
            print(f"Advertencia: Video {video} está en GT pero no en predicciones. Se omitirá.")
            continue
            
        gt_labels = gt_dict[video]
        pred_labels = preds_dict[video]
        
        # Alinear largos en caso de diferencias menores por redondeo de FPS
    ...
    if not all_gts:
        print("Error: No hay datos para evaluar. Verifica que los nombres de los videos coincidan.")
        return
        
    # Calcular Frame-wise Accuracy (MoF)
    mof = accuracy_score(all_gts, all_preds)
    
    print("\n" + "="*50)
    print("RESULTADOS DE SEGMENTACIÓN DE ACCIONES")
    print("="*50)
    print(f"Total de frames evaluados: {len(all_gts)}")
    print(f"Frame-wise Accuracy (MoF): {mof * 100:.2f}%")
    print("\nDesglose por clase (F1-Score, Precision, Recall):")
    print("-" * 50)
    
    # Classification report maneja las clases detectadas automáticamente
    report = classification_report(all_gts, all_preds, zero_division=0)
    print(report)

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Evaluación de Segmentación Temporal Densa")
        print(f"Uso: {sys.argv[0]} [predicciones_densas.txt] [ground_truth_denso.txt]")
        print("Formato esperado por línea: video_name \\t frame_idx \\t label_name")
        sys.exit(1)

    filename_preds = sys.argv[1]
    filename_gt = sys.argv[2]
    
    evaluar_segmentacion(filename_preds, filename_gt)
